...
@document.command("import-svg")
@click.argument("svg_path")
@click.option("--name", "name", type=str, default=None, help="Override imported document name")
@click.option("--output", "output_path", type=str, required=True, help="Save imported project JSON to this path")
@handle_error
def document_import_svg(svg_path, name, output_path):
    """Import an existing SVG into an editable project JSON file."""
    proj = doc_mod.import_svg_document(svg_path, name=name)
    doc_mod.save_document(proj, output_path)
    sess = get_session()
    sess.set_project(proj, output_path)
    info = doc_mod.get_document_info(proj)
    output(info, f"Imported SVG: {svg_path}")
