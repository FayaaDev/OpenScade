...
For repo-local usage without installing a console script, use:

```bash
./tools/cli-anything-inkscape --help
```
...
python3 -m cli_anything.inkscape document new --name "MyDrawing" -o drawing.json

# Import an existing SVG into an editable project
python3 -m cli_anything.inkscape document import-svg existing.svg --output imported.json

# Add shapes
python3 -m cli_anything.inkscape --project drawing.json shape add-rect --x 100 --y 100 --width 200 --height 150
python3 -m cli_anything.inkscape --project drawing.json shape add-circle --cx 400 --cy 300 --r 80
python3 -m cli_anything.inkscape --project drawing.json shape add-star --cx 700 --cy 300 --points 5 --outer-r 100

# Style objects
python3 -m cli_anything.inkscape --project drawing.json style set-fill 0 "#ff0000"
python3 -m cli_anything.inkscape --project drawing.json style set-stroke 1 "#000000" --width 3

# Add text
python3 -m cli_anything.inkscape --project drawing.json text add --text "Hello World" --x 100 --y 50 --font-size 36

# Transform objects
python3 -m cli_anything.inkscape --project drawing.json transform translate 0 50 --ty 25
python3 -m cli_anything.inkscape --project drawing.json transform rotate 1 45

# Add gradients
python3 -m cli_anything.inkscape --project drawing.json gradient add-linear --color1 "#ff0000" --color2 "#0000ff"
python3 -m cli_anything.inkscape --project drawing.json gradient apply 0 0

# Export
python3 -m cli_anything.inkscape --project drawing.json export svg output.svg --overwrite
python3 -m cli_anything.inkscape --project drawing.json export png output.png --overwrite
```
...
```bash
python3 -m cli_anything.inkscape --json document new -o doc.json
python3 -m cli_anything.inkscape --json --project doc.json shape list
```

## Interactive REPL

```bash
python3 -m cli_anything.inkscape repl
# or with existing project:
python3 -m cli_anything.inkscape repl --project doc.json
```
...
document import-svg  - Import an existing SVG into project JSON
