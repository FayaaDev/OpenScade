...
from cli_anything.inkscape.core.document import (
    ...
    project_to_svg, save_svg, import_svg_document,
)
...
class TestDocument:
    ...
    def test_import_svg_document_preserves_basic_shapes_and_layers(self, tmp_path):
        svg_path = tmp_path / "import_me.svg"
        svg_path.write_text(
            """<?xml version=\"1.0\" encoding=\"UTF-8\"?>
<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:inkscape=\"http://www.inkscape.org/namespaces/inkscape\" width=\"320px\" height=\"240px\" viewBox=\"0 0 320 240\">
  <defs>
    <linearGradient id=\"grad1\"><stop offset=\"0\" style=\"stop-color:#ff0000;stop-opacity:1\"/><stop offset=\"1\" style=\"stop-color:#0000ff;stop-opacity:1\"/></linearGradient>
  </defs>
  <g id=\"foreground\" inkscape:groupmode=\"layer\" inkscape:label=\"Foreground\">
    <rect id=\"hero\" x=\"10\" y=\"20\" width=\"90\" height=\"40\" style=\"fill:url(#grad1);stroke:#111111;stroke-width:2\" />
    <text id=\"title\" x=\"40\" y=\"80\" style=\"font-family:Inter;font-size:18px;fill:#222222\">Hello import</text>
  </g>
</svg>
""",
            encoding="utf-8",
        ...
        proj = import_svg_document(str(svg_path))

        assert proj["name"] == "import_me"
        assert proj["document"]["width"] == 320
        assert proj["document"]["height"] == 240
        assert proj["document"]["background"] == "none"
        assert len(proj["gradients"]) == 1
        assert any(layer["name"] == "Foreground" for layer in proj["layers"])
        assert any(obj["id"] == "hero" and obj["type"] == "rect" for obj in proj["objects"])
        imported_text = next(obj for obj in proj["objects"] if obj["id"] == "title")
        assert imported_text["type"] == "text"
        assert imported_text["text"] == "Hello import"
        assert imported_text["font_family"] == "Inter"
