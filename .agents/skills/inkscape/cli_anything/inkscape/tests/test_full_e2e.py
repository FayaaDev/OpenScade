...
from cli_anything.inkscape.core.document import (
    ...
    get_document_info, project_to_svg, import_svg_document,
)
...
class TestCLISubprocess:
    ...
    def test_document_import_svg(self, tmp_dir):
        svg_path = os.path.join(tmp_dir, "existing.svg")
        project_path = os.path.join(tmp_dir, "imported.json")
        with open(svg_path, "w", encoding="utf-8") as f:
            f.write(
                """<?xml version=\"1.0\" encoding=\"UTF-8\"?>
<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"120\" height=\"90\">
  <rect id=\"box\" x=\"5\" y=\"6\" width=\"30\" height=\"40\" fill=\"#ff0000\" />
</svg>
"""
            )

        result = self._run(["document", "import-svg", svg_path, "--output", project_path])
        assert result.returncode == 0
        assert os.path.exists(project_path)
        imported = open_document(project_path)
        assert imported["document"]["width"] == 120
        assert any(obj["id"] == "box" for obj in imported["objects"])
