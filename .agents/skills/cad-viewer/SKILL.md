---
name: cad-viewer
description: Start or reuse CAD Viewer and return review links for explicit CAD, robot-description, and G-code files. Use after OpenSCAD STL export in this repo, and when visually reviewing `.step`, `.stp`, `.glb`, `.stl`, `.3mf`, `.gcode`, `.dxf`, `.urdf`, `.srdf`, or `.sdf` files.
---
...
Start or reuse one local CAD Viewer with `npm run start`, then select
...
For this repo's OpenSCAD workflow, STL exports usually live under `exports/` or
`Printed/`; set `?dir=` to the absolute exported-artifact directory that owns the
STL files being reviewed.
...
```bash
npm --prefix scripts/viewer run start -- --host 127.0.0.1 --port 4178 --shutdown-after 12h
```
...
If a non-Viewer process occupies the candidate port, rerun `start` with
...
the same `npm --prefix scripts/viewer run start -- --host 127.0.0.1 --port
